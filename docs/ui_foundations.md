...
The engine layout also renders an accessible breadcrumb trail for resource pages. Collection pages show the current resource collection; new, show, and edit pages link back through the collection and record before marking the current action with `aria-current="page"`. Record labels use the first readable show field and fall back to the resource label.
