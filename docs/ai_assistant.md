...
`ActiveRecordTraceStore` supplies the durable trace-record contract with tenant-first query behavior and `[redacted]` output retention by default. Hosts configure it explicitly through `KrudminAI.config.ai_trace_store`, supply the model and retention policy, and must still add authenticated trace search/export delivery and their selected background provider job.
...
`ActiveRecordProposalStore` persists a tenant-bound `pending_review` draft, redacts retained evidence, records the reviewer when the same tenant approves it, and only resolves `approved` proposals for that tenant. It is a storage contract, not mutation authority: an approved proposal must still pass the host approval policy, canonical record lookup, field authorization, normal validation, audit, and pre-mutation trace through `ApprovedAutomation`.