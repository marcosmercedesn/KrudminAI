...
The canonical query pipeline is tenant scope, policy scope, filters, sort, pagination. Every list, search, export, dashboard, and AI context builder must use it. The implemented resource and pipeline contracts are documented in [resource_query_pipeline.md](resource_query_pipeline.md). The future mutation pipeline dispatches create, update, destroy, transition, and custom-action commands through explicit authorization, normalized results, response adapters, and audit hooks.
