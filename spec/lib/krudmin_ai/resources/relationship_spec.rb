  ...
  it "declares a has-one relationship with singular cardinality" do
    resource = Class.new(described_class) do
      has_one :insurance,
        ...
        authorize: ->(_record, _action, _context) { true },
        tenant_record: ->(_record, _context) { true },
        field_authorizers: { provider: { read: ->(_record, _context) { true }, write: ->(_record, _context) { true } } }
    end

    relationship = resource.relationships.fetch(:insurance)
    expect(relationship).to be_singular
    expect(relationship.maximum).to eq(1)
    expect(resource.nested_permitted_attributes).to eq([ { insurance_attributes: %i[id _destroy provider] } ])
  end
end
