  ...
  module Fields
    class HasManyIds < BelongsTo
      def form_control(form, writable:, errors:, access_note_id:, context:, authorization_provider: nil, rules: {}, error_id: nil)
        form.collection_select(attribute, options_for(context, authorization_provider:), :last, :first, {}, multiple: true, **control_options(writable:, errors:, access_note_id:, error_id:, rules:, class_name: "krudmin-ai-select"))
      end
      ...
      def validation_constraints = { multiple: true }
